...
This file defines **how coding agents should work in this Turborepo monorepo**.
It applies repo-wide to `apps/*` and `packages/*`.

> If a subfolder has its own `AGENTS.md` or agent-specific guide, follow the
...
6. During secondary README consistency checks, keep the status line brief and
   use the standard green-path wording when no issues are reproduced.

### Delivery format

- In the required root `REVIEW.md`, the `## Uso de IA` section may include a
  short `### Validación cruzada` subsection.
- Keep that subsection concise and include these labels when applicable:
  `Instrucciones locales revisadas`, `Contraste manual`, and `Limitaciones del apoyo`.
...
- End `REVIEW.md` with the footer:
  `Avila Tek 2026 - Marca registrada.`
